<!DOCTYPE html>
<html>
	<head>
		<meta charset="utf-8">
		<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
		<title>Our Portfolio</title>
		<meta name="description" content="Browse DigiTroopers' portfolio to see our successful projects in SEO, web development, and digital marketing across various industries.">
		<!-- <link rel="canonical" href="https://digitroopers.com/" />
		<meta property="og:title" content="DigiTroopers | Digital Marketing & Growth Agency" />
		<meta property="og:description" content="We help businesses grow with SEO, Social Media Marketing & Web Development. Let’s turn your potential into profit." />
		<meta property="og:url" content="https://digitroopers.com/" />
		<meta property="og:type" content="website" />
		<meta property="og:image" content="assets/images/logo/favicon.webp" />
		<meta name="twitter:card" content="summary_large_image" />
		<meta name="twitter:title" content="DigiTroopers | Digital Marketing & Growth Agency" />
		<meta name="twitter:description" content="We help businesses grow with SEO, SMM & Web Development. Let’s make your brand shine." />
		<meta name="twitter:image" content="assets/images/logo/favicon.webp" /> -->
		<link rel="shortcut icon" href="assets/images/logo/favicon.webp">
		<link rel="stylesheet" href="assets/css/jost.css" >
		<link rel="stylesheet" href="assets/vendors/font-awesome/all.min.css">
		<link rel="stylesheet" href="assets/vendors/bootstrap/css/bootstrap.min.css">
		<link rel="stylesheet" href="assets/vendors/bootstrap/bootstrap-icons.min.css">
		<link rel="stylesheet" href="assets/vendors/owl-carousel/owl.carousel.min.css">
		<link rel="stylesheet" href="assets/vendors/owl-carousel/owl.theme.default.min.css">
		<link rel="stylesheet" href="assets/vendors/lenis/lenis.css">
		<link rel="stylesheet" href="assets/vendors/swiper/swiper.min.css">
		<link rel="stylesheet" href="assets/vendors/magnific-popup/magnific-popup.css">
		<link rel="stylesheet" href="assets/vendors/slideshow/subtle-slideshow.css">
		<link rel="stylesheet" href="assets/css/style.css">
		<link rel="stylesheet" href="assets/css/themes.css">
		<link rel="stylesheet" href="assets/css/responsive1.css">
		<link rel="stylesheet" href="assets/css/responsive.css">
        <!-- Meta Pixel Code -->
		<script>
			!function(f,b,e,v,n,t,s)
			{if(f.fbq)return;n=f.fbq=function(){n.callMethod?
			n.callMethod.apply(n,arguments):n.queue.push(arguments)};
			if(!f._fbq)f._fbq=n;n.push=n;n.loaded=!0;n.version='2.0';
			n.queue=[];t=b.createElement(e);t.async=!0;
			t.src=v;s=b.getElementsByTagName(e)[0];
			s.parentNode.insertBefore(t,s)}(window, document,'script',
			'https://connect.facebook.net/en_US/fbevents.js');
			fbq('init', '596119242764284');
			fbq('track', 'PageView');
		</script>

		<noscript>
			<img height="1" width="1" style="display:none" src="https://www.facebook.com/tr?id=596119242764284&ev=PageView&noscript=1" />
		</noscript>
		<!-- End Meta Pixel Code -->

		<!-- Google tag (gtag.js) -->
		<script async src="https://www.googletagmanager.com/gtag/js?id=G-Y5E91ZZDGJ"></script>

		<script>
			window.dataLayer = window.dataLayer || [];
			function gtag(){dataLayer.push(arguments);}
			gtag('js', new Date());
			gtag('config', 'G-Y5E91ZZDGJ');
		</script>
		
		<script type="application/ld+json">
			{
				"@context": "https://schema.org",
				"@type": "Product",
				"name": "E-Commerce Management Company USA | Digitroopers",
				"aggregateRating": {
				"@type": "AggregateRating",
				"ratingValue": "4.9",
				"reviewCount": "557"
				}
			}
		</script>
		<!-- Google Tag Manager (noscript) -->
		<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-TFQVC8Q" height="0" width="0" style="display:none;visibility:hidden"></iframe></noscript>
		<!-- End Google Tag Manager (noscript) -->
	</head>	

    <body>
        <!-- Start Cursor -->
		<div id="cursor">
			<div class="cursor__circle"></div>
		</div>
		<!-- End Cursor -->

        <div id="page" class="main">
            <!-- Start Header -->
            <header class="header header-sticky">
                <nav class="navbar navbar-static-top navbar-expand-lg">
                    <div class="container-fluid main-header position-relative">
                        <a class="navbar-brand" href="index">
                            <img class="img-fluid" src="assets/images/logo/logo.png" alt="Digitroopers Logo" loading="eager">
                        </a>
                        <div class="navbar-collapse collapse">
                            <ul class="navbar-nav">
                                <li class="nav-item">
                                    <a class="nav-link" href="index">
                                        <span>Home</span>
                                    </a>
                                </li>
                                <li class="nav-item dropdown dropdown-mega position-static">
                                    <a class="nav-link dropdown-toggle" href="javascript:void(0)" data-bs-toggle="dropdown">
                                        About Us
                                        <i class="fa-solid fa-chevron-down"></i>
                                    </a>
                                    <div class="dropdown-menu shadow">
                                        <div class="mega-content">
                                            <div class="container">
                                                <div class="row">
                                                    <div class="col-md-3 megamenu">
                                                        <h5 class="megamenu-heading">About Us</h5>
                                                        <div class="menu-item-description">A full-cycle software and mobile app development company with a world-class team of innovators.</div>
                                                    </div>
                                                    <div class="col-md-3 megamenu">
                                                        <h5 class="megamenu-heading">Explore</h5>
                                                        <ul>
                                                            <li>
                                                                <a href="about-us">
                                                                    <div class="menu-item-title">About Us</div>
                                                                    <div class="menu-item-description">About Us</div>
                                                                </a>
                                                            </li>
                                                            <li class="pt-4">
                                                                <a href="our-process">
                                                                    <div class="menu-item-title">Our Process</div>
                                                                    <div class="menu-item-description">Our Agile Process</div>
                                                                </a>
                                                            </li>
                                                        </ul>
                                                    </div>
                                                    <div class="col-md-3 megamenu">
                                                        <h5 class="megamenu-heading">&nbsp;</h5>
                                                        <ul>
                                                            <!-- <li>
                                                                <a href="https://digitroopers.com/testimonial">
                                                                    <div class="menu-item-title">Testimonials</div>
                                                                    <div class="menu-item-description">What our clients have to say</div>
                                                                </a>
                                                            </li> -->
                                                            <li class="">
                                                                <a href="clients">
                                                                    <div class="menu-item-title">Clients</div>
                                                                    <div class="menu-item-description">Our Global Clients</div>
                                                                </a>
                                                            </li>
                                                            <li class="pt-4">
                                                                <a href="career">
                                                                    <div class="menu-item-title">Careers</div>
                                                                    <div class="menu-item-description">Build your careers with us</div>
                                                                </a>
                                                            </li>
                                                        </ul>
                                                    </div>
                                                    <div class="col-md-3 megamenu group-dark">
                                                        <div class="row my-auto">
                                                            <div class="col-md-6">
                                                                <div class="companydetailmenu">
                                                                    <h4>12+</h4>
                                                                    <h5>Years</h5>
                                                                </div>
                                                            </div>
                                                            <div class="col-md-6">
                                                                <div class="companydetailmenu">
                                                                    <h4>200+</h4>
                                                                    <h5>Employees</h5>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                                <li class="nav-item dropdown dropdown-mega position-static">
                                    <a class="nav-link dropdown-toggle" href="javascript:void(0)" data-bs-toggle="dropdown">Services
                                        <i class="fa-solid fa-chevron-down"></i>
                                    </a>
                                    <div class="dropdown-menu shadow">
                                        <div class="mega-content">
                                            <div class="container">
                                                <div class="row">
                                                    <div class="col-lg-3 megamenu">
                                                        <h5 class="megamenu-heading">Our Services</h5>
                                                        <p>Leveraging cutting-edge technologies to build industry-leading digital products.</p>
                                                    </div>
                                                    <div class="col-lg-3 megamenu">
                                                        <ul class="list-unstyled">
                                                            <li>
                                                                <a class="dropdown-item" href="graphic-design">
                                                                    <div class="menu-item-title">Designing</div>
                                                                    <div class="menu-item-description">Logo, Digital-Banner, Web(UI/UX), Business Card</div>
                                                                </a>
                                                            </li>
                                                            <li>
                                                                <a class="dropdown-item" href="paid-marketing">
                                                                    <div class="menu-item-title">PPC Managment Services</div>
                                                                    <div class="menu-item-description">Search Ads, Display Ads, Video Ads and Shopping Ads</div>
                                                                </a>
                                                            </li>
                                                            <li>
                                                                <a class="dropdown-item" href="paid-marketing">
                                                                    <div class="menu-item-title">E-Commerce Project Managment</div>
                                                                    <div class="menu-item-description">B2B, B2C, C2C and C2B</div>
                                                                </a>
                                                            </li>
                                                            <li>
                                                                <a class="dropdown-item" href="animation">
                                                                    <div class="menu-item-title">Animation</div>
                                                                    <div class="menu-item-description">2D 3D Animation</div>
                                                                </a>
                                                            </li>
                                                        </ul>
                                                    </div>
                                                    <div class="col-lg-3 megamenu">
                                                        <ul class="list-unstyled">
                                                            <li>
                                                                <a class="dropdown-item" href="website-development">
                                                                    <div class="menu-item-title">Web Development</div>
                                                                    <div class="menu-item-description">Front-End, Back-End and Full-Stack</div>
                                                                </a>
                                                            </li>
                                                            <li>
                                                                <a class="dropdown-item" href="social-media-marketing">
                                                                    <div class="menu-item-title">Social Media Marketing</div>
                                                                    <div class="menu-item-description">Social Networking, Blogging/Community Building</div>
                                                                </a>
                                                            </li>
                                                            <li>
                                                                <a class="dropdown-item" href="email-marketing">
                                                                    <div class="menu-item-title">Email Marketing</div>
                                                                    <div class="menu-item-description">Setup Email and Community Management</div>
                                                                </a>
                                                            </li>
                                                            <li>
                                                                <a class="dropdown-item" href="seo">
                                                                    <div class="menu-item-title">SEO Management</div>
                                                                    <div class="menu-item-description">On-Page, Off-Page, Technical</div>
                                                                </a>
                                                            </li>
                                                        </ul>
                                                    </div>
                                                    <div class="col-lg-3 megamenu dark-group">
                                                        <h5 class="megamenu-heading">How We Work</h5>
                                                        <div class="mt-2">
                                                            <span class="fw-bold text-primary">Idea Analysis</span>
                                                            <p class="p-black mb-0">Exploring project ideas</p>
                                                        </div>
                                                        <div class="mt-2">
                                                            <span class="fw-bold text-primary">Wireframing and Architectural Planning</span>
                                                            <p class="p-black mb-0">Project wireframe and planning</p>
                                                        </div>
                                                        <div class="mt-2">
                                                            <span class="fw-bold text-primary">Functional Model Development</span>
                                                            <p class="p-black mb-0">Functional development process</p>
                                                        </div>
                                                        <div class="mt-2">
                                                            <span class="fw-bold text-primary">Deployment</span>
                                                            <p class="p-black mb-0">Full-function product delivery</p>
                                                        </div>
                                                        <div class="mt-2">
                                                            <span class="fw-bold text-primary">Quality Assurance</span>
                                                            <p class="p-black mb-0">Incremental testing and QA Deployment</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                                <li class="nav-item active">
                                    <a class="nav-link" href="portfolio">
                                        <span>Portfolio</span>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link" href="package">
                                        <span>Packages</span>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link" href="https://blog.digitroopers.com/">
                                        <span>Insight</span>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link" href="contactus">
                                        <span>Contact Us</span>
                                    </a>
                                </li>
                            </ul>
                        </div>
                        <div class="action-group">
                            <a class="menu-btn hamburger" href="#" data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight" aria-controls="offcanvasRight">
                                <span class="hamburger-container">
                                    <span class="hamburger-inner"></span>
                                    <span class="hamburger-hidden"></span>
                                </span>
                            </a>
                            <div class="header-phone-number">
                                <i class="fa-solid fa-phone-volume"></i>
                                <a href="tel:+966507438993">+966-507438993</a>
                            </div>
                            <a class="light" href="#" title="Change Theme"></a>
                            <div class="mobile-action d-block d-lg-none">
                                <div class="mobile-hamburger">
                                    <a class="hamburger" href="#" data-bs-toggle="offcanvas" data-bs-target="#menuOffcanvas" aria-controls="menuOffcanvas">
                                        <span class="hamburger-container">
                                            <span class="hamburger-inner"></span>
                                            <span class="hamburger-hidden"></span>
                                        </span>
                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                </nav>
            </header>
            <!-- End Header -->

            <!-- Start Main Menu OffCanvas -->
            <div class="offcanvas main-menu-offcanvas offcanvas-end" tabindex="-1" id="menuOffcanvas">
                <div class="offcanvas-header">
                    <p class="offcanvas-title text-white fs-2" id="offcanvasNavbar2Label">Main Menu</p>
                    <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close">
                        <i class="fa-solid fa-xmark"></i>
                    </button>
                </div>
                <div class="offcanvas-body lenis-scroll-disable">
                    <div class="body-inner h-100 pe-3 lenis-scroll-disable">
                        <nav class="navbar">
                            <ul class="nav navbar-nav">
                                <li class="nav-item">
                                    <a class="nav-link" href="index">
                                        <span>Home</span>
                                    </a>
                                </li>
                                <li class="dropdown nav-item">
                                    <a class="nav-link" href="javascript:void(0)" data-bs-toggle="dropdown">About Us
                                        <i class="fa-solid fa-chevron-down"></i>
                                    </a>
                                    <ul class="dropdown-menu megamenu dropdown-menu-lg">
                                        <li>
                                            <a class="dropdown-item" href="about-us">
                                                <span>About Us </span> 
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="our-process">
                                                <span>Our Process</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="clients">
                                                <span>Clients</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="career">
                                                <span>Careers</span>
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                                <li class="dropdown nav-item active">
                                    <a class="nav-link" href="javascript:void(0)" data-bs-toggle="dropdown">Services
                                        <i class="fa-solid fa-chevron-down"></i>
                                    </a>
                                    <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                                        <li>
                                            <a class="dropdown-item" href="graphic-design">
                                                <span>Designing</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="website-development">
                                                <span>Web development</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="social-media-marketing">
                                                <span>Social Media Marketing</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="seo">
                                                <span>SEO Managment</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="email-marketing">
                                                <span>E-mail Marketing</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="paid-marketing">
                                                <span>PPC Managment Services</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="ecommerce">
                                                <span>E-Commerce Project Managment</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="animation">
                                                <span>Animation</span>
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link active" href="portfolio">
                                        <span>Portfolio</span>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link" href="package">
                                        <span>Packages</span>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link" href="https://blog.digitroopers.com/">
                                        <span>Insight</span>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link" href="contactus">
                                        <span>Contact Us</span>
                                    </a>
                                </li>
                            </ul>
                        </nav>
                    </div>
                </div>
            </div>
            <!-- End Main Menu OffCanvas -->

            <!-- Start Search -->
            <div id="search">
                <button type="button" class="close"></button>
                <div class="search-inner">
                    <h2 class="title">Search here...</h2>
                    <form>
                        <input type="search" value="" placeholder="Search">
                        <button type="submit" class="search-btn btn"><span>Search</span></button>
                    </form>
                </div>
            </div>
            <!-- End Search -->

            <!-- Start Right Menu -->
            <div class="offcanvas offcanvas-end header-sidepanel bg-white" tabindex="-1" id="offcanvasRight">
                <div class="offcanvas-header text-end justify-content-end">
                    <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close">
                        <i class="fa-solid fa-xmark"></i>
                    </button>
                </div>
                <div class="offcanvas-body lenis-scroll-disable">
                    <div class="sidebar-menu h-100 d-flex flex-column">
                        <div class="about-us-box">
                            <p class="title">Scalable Business for Startups</p>
                            <p>Get the oars in the water and start rowing. Execution is the single biggest factor in achievement so the faster and better your execution.</p>
                        </div>

                        <div class="widget-social">
                            <ul class="social-icon">
                                <li>
                                    <i class="icon">
                                        <svg width="22" height="22" fill="#23a455" viewBox="0 0 320 512">
                                            <path d="M279.14 288l14.22-92.66h-88.91v-60.13c0-25.35 12.42-50.06 52.24-50.06h40.42V6.26S260.43 0 225.36 0c-73.22 0-121.08 44.38-121.08 124.72v70.62H22.89V288h81.39v224h100.17V288z"></path>
                                        </svg>
                                    </i>
                                    <a href="https://www.facebook.com/DigiTroopers/">Facebook</a>
                                </li>
                                <li>
                                    <i class="icon">
                                        <svg width="22" height="22" fill="#23a455" viewBox="0 0 16 16">
                                            <path d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.9 3.9 0 0 0-1.417.923A3.9 3.9 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.9 3.9 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.9 3.9 0 0 0-.923-1.417A3.9 3.9 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599s.453.546.598.92c.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.5 2.5 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.5 2.5 0 0 1-.92-.598 2.5 2.5 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233s.008-2.388.046-3.231c.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92s.546-.453.92-.598c.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92m-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217m0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334"></path>
                                        </svg>
                                    </i>
                                    <a href="https://www.instagram.com/digitalmarketingservicesusa/">Instagram</a>
                                </li>
                                <li>
                                    <i class="icon">
                                        <svg width="22" height="22" fill="#23a455" viewBox="0 0 512 512">
                                            <path d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8L200.7 275.5 26.8 48H172.4L272.9 180.9 389.2 48zM364.4 421.8h39.1L151.1 88h-42L364.4 421.8z"></path>
                                        </svg>
                                    </i>
                                    <a href="https://x.com/Digi_Troopers/">Twitter</a>
                                </li>
                                <li>
                                    <i class="icon">
                                        <svg width="22" height="22" fill="#0A66C2" viewBox="0 0 24 24"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.026-3.037-1.851-3.037-1.851 0-2.134 1.445-2.134 2.939v5.667H9.353V9h3.413v1.561h.049c.476-.899 1.637-1.85 3.369-1.85 3.6 0 4.264 2.369 4.264 5.455v6.286zM5.337 7.433a2.062 2.062 0 1 1 0-4.124 2.062 2.062 0 0 1 0 4.124zM6.778 20.452H3.893V9h2.885v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.226.792 24 1.771 24h20.451C23.2 24 24 23.226 24 22.271V1.729C24 .774 23.2 0 22.222 0z"></path>
                                        </svg>
                                    </i>
                                    <a href="https://www.linkedin.com/company/digitroopers/">Linkedin</a>
                                </li>
                            </ul>
                        </div>

                        <div class="contact-info-box">
                            <p class="title">Contact Details</p>
                            <ul>
                                <li>
                                    <a href="mailto:info@digitroopers.com">→ info@digitroopers.com</a>
                                </li>
                                <li>
                                    <a href="tel:→1(253)5284057">→ +1 (253) 528-4057</a>
                                </li>
                                <li>→ 12316 SE 254th Ct Kent, WA 98030, United States</li>
                                <li>→ Floor 3, Building 7264, Ibn Al Fourat, Jareer, P.O 12837 Riyadh, Saudi Arabia</li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
            <!-- End Right menu -->

            <div class="site-content">
                <div class="container-fluid">
                    <div class="item-efftect">
                        <div class="efftect overflow-hidden"></div>
                        <div class="efftect overflow-hidden"></div>
                        <div class="efftect overflow-hidden"></div>
                        <div class="efftect overflow-hidden"></div>
                        <div class="efftect overflow-hidden"></div>
                    </div>
                </div>

                <!-- Start Inner Banner -->
                <section>
                    <div class="inner-banner bg-holder rounded-section jarallax">
                        <div class="container">
                            <div class="row">
                                <div class="col-md-12">
                                    <h1 class="title text-white">Portfolio</h1>
                                </div>
                            </div>
                        </div>
                    </div>
                </section>
                <!-- End Inner Banner -->

                <!-- Start Projects  -->
                <div class="content-wrapper">
                    <section class="space-ptb">
                        <div class="container">
                            <div class="top-text text-center wow fadeInUp">
                                <h2 class="text-primary">Our Projects</h2>
                                <h3 class="text-primary">Some of Our Works</h3>
                                <p>We think big and have hands in all leading technology platforms to provide you wide array of services.</p>
                            </div>

                            <div class="row justify-content-start mt-4">
                                <div class="col-lg-12 wow fadeInUp" data-wow-delay="0.1s" data-wow-duration="1500ms">
                                    <div class="filters-group">
                                        <!-- <button class="btn-filter active" data-group="WEBSITE ">All</button> --> <!-- Removed -->
                                        <button class="btn-filter active" data-group="website">Website</button>
                                        <button class="btn-filter" data-group="ui-ux">UI/UX</button>
                                        <button class="btn-filter" data-group="logo">Logo</button>
                                        <button class="btn-filter" data-group="mobile-app">Mobile App</button>
                                        <button class="btn-filter" data-group="graphic">Graphic</button>
                                        <!-- <button class="btn-filter" data-group="animation">Animation</button> -->
                                        <button class="btn-filter" data-group="3d-modeling">3D Modeling</button>
                                        <button class="btn-filter" data-group="game-art">Game Art</button>
                                    </div>

                                    <div class="my-shuffle-container columns-3 full-screen case-studies-grid">
                                        <!-- Start Website -->
                                        <div class="grid-item" data-groups='["website"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/burger-house/burger-house-mockup.webp" alt="Burger House">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="burger-house">Burger House</a>
                                                            </h3>
                                                            <div class="case-studies-description">Website</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="burger-house">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["website"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/quick-mart/quick-mart-mockup.webp" alt="Quick Mart">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="quick-mart">Quick Mart</a>
                                                            </h3>
                                                            <div class="case-studies-description">Website</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="quick-mart">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["website"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/quran-hub/quran-hub-mockup.webp" alt="Quran Hub">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="quran-hub">Quran Hub</a>
                                                            </h3>
                                                            <div class="case-studies-description">Website</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="quran-hub">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["website"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/rangrez-visual/rangrez-visual-mockup.webp" alt="Rangrez Visual">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="rangrez-visual">Rangrez Visual</a>
                                                            </h3>
                                                            <div class="case-studies-description">Website</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="rangrez-visual">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["website"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/ride-smart/ride-smart-mockup.webp" alt="Ride Smart">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="ride-smart">Ride Smart</a></h3>
                                                            <div class="case-studies-description">Website</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="ride-smart">
                                                                <svg  width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["website"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/skyline-estate/skyline-estate-mockup.webp" alt="Skyline Estate">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="skyline-estate">Skyline Estate</a>
                                                            </h3>
                                                            <div class="case-studies-description">Website</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="skyline-estate">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <!-- End Website -->

                                        <!-- Start UI/UX -->
                                        <div class="grid-item" data-groups='["ui-ux"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/ui-ux/ui-ux-mockup-1.webp" alt="Burger House UI/UX">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="burger-house">Burger House</a>
                                                            </h3>
                                                            <div class="case-studies-description">UI/UX</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="burger-house">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["ui-ux"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/ui-ux/ui-ux-mockup-2.webp" alt="Quick Mart UI/UX">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="quick-mart">Quick Mart</a>
                                                            </h3>
                                                            <div class="case-studies-description">UI/UX</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="quick-mart">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["ui-ux"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/ui-ux/ui-ux-mockup-3.webp" alt="Quran Hub UI/UX">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="quran-hub">Quran Hub</a>
                                                            </h3>
                                                            <div class="case-studies-description">UI/UX</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="quran-hub">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["ui-ux"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/ui-ux/ui-ux-mockup-4.webp" alt="Skyline Estate UI/UX">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a  href="skyline-estate">Skyline Estate</a>
                                                            </h3>
                                                            <div class="case-studies-description">UI/UX</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="skyline-estate">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["ui-ux"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/ui-ux/ui-ux-mockup-5.webp" alt="Rangrez Visual UI/UX">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="rangrez-visual">Rangrez Visual</a>
                                                            </h3>
                                                            <div class="case-studies-description">UI/UX</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="rangrez-visual">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["ui-ux"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/food-app/food-app-mockup.webp" alt="Food App UI/UX">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="food-ui-ux">Food App</a>
                                                            </h3>
                                                            <div class="case-studies-description">UI/UX</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="food-ui-ux">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["ui-ux"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/travel-guide/travel-guide-mockup.webp" alt="Travel Guide">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="travel-guide">Travel Guide</a>
                                                            </h3>
                                                            <div class="case-studies-description">UI/UX</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="travel-guide">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <!-- <div class="grid-item" data-groups='["ui-ux"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/travel-app/travel-app-mockup.webp" alt="Travel App UI/UX">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="travel-ui-ux">Travel App</a>
                                                            </h3>
                                                            <div class="case-studies-description">UI/UX</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="travel-ui-ux">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div> -->
                                        <!-- END UI UX -->
                                        
                                        <!-- Start Logo -->
                                        <div class="grid-item" data-groups='["logo"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/glamora/glamora-mockup.webp" alt="Glamora Brand">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                            <div class="case-studies-content">
                                                                <h3 class="case-studies-title">
                                                                    <a href="glamora">Glamora</a>
                                                                </h3>
                                                                <div class="case-studies-description">Logo</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="glamora">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["logo"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/nestify/nestify-mockup.webp" alt="Nestify Brand">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                            <div class="case-studies-content">
                                                                <h3 class="case-studies-title">
                                                                    <a href="nestify">Nestify</a>
                                                                </h3>
                                                                <div class="case-studies-description">Logo</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="nestify">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["logo"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/ecocraft/ecocraft-mockup.webp" alt="Ecocraft Brand">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                            <div class="case-studies-content">
                                                                <h3 class="case-studies-title">
                                                                    <a href="ecocraft">Ecocraft</a>
                                                                </h3>
                                                                <div class="case-studies-description">Logo</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="ecocraft">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["logo"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/rapid-haul/rapid-haul-mockup.webp" alt="Rapid Haul Brand">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="rapid-haul">Rapid Haul</a>
                                                            </h3>
                                                            <div class="case-studies-description">Logo</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="rapid-haul">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["logo"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/bun-bawk/bun-bawk-mockup.webp" alt="Bun & Bawk Brand">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="bun-and-bawk">Bun & Bawk</a>
                                                            </h3>
                                                            <div class="case-studies-description">Logo</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="bun-and-bawk">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["logo"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/pixel-pro/pixel-pro-mockup.webp" alt="PixelPro Brand">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="pixel-pro">PixelPro</a>
                                                            </h3>
                                                            <div class="case-studies-description">Logo</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="pixel-pro">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["logo"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/vibeta/vibeta-mockup.webp" alt="Vibeta Brand">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="vibeta">Vibeta</a>
                                                            </h3>
                                                            <div class="case-studies-description">Logo</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="vibeta">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["logo"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/estaty/estaty-mockup.webp" alt="Estaty Brand">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="estaty">Estaty</a>
                                                            </h3>
                                                            <div class="case-studies-description">Logo</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="estaty">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["logo"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/ice-popz/ice-popz-mockup.webp" alt="IcePopz Brand">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="ice-popz">Ice Popz</a>
                                                            </h3>
                                                            <div class="case-studies-description">Logo</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="ice-popz">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["logo"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/rang-o-nigar/rang-o-nigar-mockup.webp" alt="Rang-o-Nigar Brand">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="rang-o-nigar">Rang-o-Nigar</a>
                                                            </h3>
                                                            <div class="case-studies-description">Logo</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="rang-o-nigar">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["logo"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/lams-e-husun/lams-e-husun-mockup.webp" alt="Lams e Husun Brand">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="lams-e-husun">Lams e Husun</a>
                                                            </h3>
                                                            <div class="case-studies-description">Logo</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="lams-e-husun">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <!-- End Logo -->

                                        <!-- Start Mobile App -->
                                        <div class="grid-item" data-groups='["mobile-app"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/reborn/reborn-mockup.webp" alt="Reborn">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="reborn">Reborn</a>
                                                            </h3>
                                                            <div class="case-studies-description">Mobile App</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="reborn">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["mobile-app"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/beat-house/beat-house-mockup.webp" alt="Beat House">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="beat-house">Beat House</a>
                                                            </h3>
                                                            <div class="case-studies-description">Mobile App</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="beat-house">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["mobile-app"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/fresh-cart/fresh-cart-mockup.webp" alt="Fresh Cart">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="fresh-cart">Fresh Cart</a>
                                                            </h3>
                                                            <div class="case-studies-description">Mobile App</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="fresh-cart">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["mobile-app"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/ar-furniture/ar-furniture-mockup.webp" alt="AR Furniture">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="ar-furniture">AR Furniture</a>
                                                            </h3>
                                                            <div class="case-studies-description">Mobile App</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="ar-furniture">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["mobile-app"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/glowe/glowe-mockup.webp" alt="Glowe">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="glowe">Glowe</a>
                                                            </h3>
                                                            <div class="case-studies-description">Mobile App</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="glowe">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <!--End Mobile App -->

                                        <!-- Start Graphics -->
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-01.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-02.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-03.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-04.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-05.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-06.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-07.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-08.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-09.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-10.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-11.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-12.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-13.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-14.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-15.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-16.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-17.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-18.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-19.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-20.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-21.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-22.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-23.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-24.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-25.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-26.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-27.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-28.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-29.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-30.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-31.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-32.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-33.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-34.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-35.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-36.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-37.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-38.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-39.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-40.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-41.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-42.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["graphic"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/graphics/graphics-mockup-43.webp" alt="Graphics">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Branding</a>
                                                            </h3>
                                                            <div class="case-studies-description">Graphic</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <!-- End Graphics -->

                                        <!-- Start Animation
                                        <div class="grid-item" data-groups='["Animation"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <iframe width="100%" height="400px" src="https://www.youtube.com/embed/29I3SVDJigY" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen frameborder="0"></iframe>
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <h3 class="case-studies-title"><a
                                                            href="https://www.youtube.com/embed/29I3SVDJigY">Futurealiti Ideas into reality</a></h3>
                                                            <div class="case-studies-content">
                                                                <div class="case-studies-description">Animation</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="https://www.youtube.com/embed/29I3SVDJigY"><svg
                                                                width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                <path
                                                                d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28"
                                                                stroke="black" stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"></path>
                                                                </svg></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["Animation"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <iframe width="100%" height="400px" src="https://www.youtube.com/embed/7yMvKPPqXJg" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen frameborder="0"></iframe>
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <h3 class="case-studies-title"><a
                                                            href="https://www.youtube.com/embed/7yMvKPPqXJg">Habib - Jeeto Hath Ke Hath</a></h3>
                                                            <div class="case-studies-content">
                                                                <div class="case-studies-description">Animation</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="https://www.youtube.com/embed/7yMvKPPqXJg"><svg
                                                                width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                <path
                                                                d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28"
                                                                stroke="black" stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"></path>
                                                                </svg></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["Animation"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <iframe width="100%" height="400px" src="https://www.youtube.com/embed/gh6exbR47FQ" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen frameborder="0"></iframe>
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <h3 class="case-studies-title"><a
                                                            href="https://www.youtube.com/embed/gh6exbR47FQ">3d- Scada</a></h3>
                                                            <div class="case-studies-content">
                                                                <div class="case-studies-description">Animation</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="https://www.youtube.com/embed/gh6exbR47FQ"><svg
                                                                width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                <path
                                                                d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28"
                                                                stroke="black" stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"></path>
                                                                </svg></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["Animation"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <iframe width="100%" height="400px" src="https://www.youtube.com/embed/CCbW_bHsuks" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen frameborder="0"></iframe>
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <h3 class="case-studies-title"><a
                                                            href="https://www.youtube.com/embed/CCbW_bHsuks">Wild Lion Simulator</a></h3>
                                                            <div class="case-studies-content">
                                                                <div class="case-studies-description">Animation</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="https://www.youtube.com/embed/CCbW_bHsuks"><svg
                                                                width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                <path
                                                                d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28"
                                                                stroke="black" stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"></path>
                                                                </svg></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["Animation"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <iframe width="100%" height="400px" src="https://www.youtube.com/embed/DAG-lvPrAd0" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen frameborder="0"></iframe>
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <h3 class="case-studies-title"><a
                                                            href="https://www.youtube.com/embed/DAG-lvPrAd0">Wild Lion Simulator</a></h3>
                                                            <div class="case-studies-content">
                                                                <div class="case-studies-description">Animation</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="https://www.youtube.com/embed/DAG-lvPrAd0"><svg
                                                                width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                <path
                                                                d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28"
                                                                stroke="black" stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"></path>
                                                                </svg></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["Animation"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <iframe width="100%" height="400px" src="https://www.youtube.com/embed/jLv0G7e3w38" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen frameborder="0"></iframe>
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <h3 class="case-studies-title"><a
                                                            href="https://www.youtube.com/embed/jLv0G7e3w38">TVC Animation</a></h3>
                                                            <div class="case-studies-content">
                                                                <div class="case-studies-description">Animation</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="https://www.youtube.com/embed/jLv0G7e3w38"><svg
                                                                width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                <path
                                                                d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28"
                                                                stroke="black" stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"></path>
                                                                </svg></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["Animation"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <iframe width="100%" height="400px" src="https://www.youtube.com/embed/4R_hPiLd2Ck" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen frameborder="0"></iframe>
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <h3 class="case-studies-title"><a
                                                            href="https://www.youtube.com/embed/4R_hPiLd2Ck">Fully Automatic Packaging Robots</a></h3>
                                                            <div class="case-studies-content">
                                                                <div class="case-studies-description">Animation</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="https://www.youtube.com/embed/4R_hPiLd2Ck"><svg
                                                                width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                <path
                                                                d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28"
                                                                stroke="black" stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"></path>
                                                                </svg></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["Animation"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <iframe width="100%" height="400px" src="https://www.youtube.com/embed/lqoKetDCOTs" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen frameborder="0"></iframe>
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <h3 class="case-studies-title"><a
                                                            href="https://www.youtube.com/embed/lqoKetDCOTs">Mafia Criminal War Game</a></h3>
                                                            <div class="case-studies-content">
                                                                <div class="case-studies-description">Animation</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="https://www.youtube.com/embed/lqoKetDCOTs"><svg
                                                                width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                <path
                                                                d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28"
                                                                stroke="black" stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"></path>
                                                                </svg></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["Animation"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <iframe width="100%" height="400px" src="https://www.youtube.com/embed/tuPTh86dc2g" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen frameborder="0"></iframe>
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <h3 class="case-studies-title"><a
                                                            href="https://www.youtube.com/embed/tuPTh86dc2g">Habib Indus Hospital TVC</a></h3>
                                                            <div class="case-studies-content">
                                                                <div class="case-studies-description">Animation</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="https://www.youtube.com/embed/tuPTh86dc2g"><svg
                                                                width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                <path
                                                                d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28"
                                                                stroke="black" stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"></path>
                                                                </svg></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["Animation"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <iframe width="100%" height="400px" src="https://www.youtube.com/embed/Z_WPpFPeLQk" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen frameborder="0"></iframe>
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <h3 class="case-studies-title"><a
                                                            href="https://www.youtube.com/embed/Z_WPpFPeLQk">Wild Leopard Simulator</a></h3>
                                                            <div class="case-studies-content">
                                                                <div class="case-studies-description">Animation</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="https://www.youtube.com/embed/Z_WPpFPeLQk"><svg
                                                                width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                <path
                                                                d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28"
                                                                stroke="black" stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"></path>
                                                                </svg></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["Animation"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <iframe width="100%" height="400px" src="https://www.youtube.com/embed/rHupXKF_IPM" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen frameborder="0"></iframe>
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <h3 class="case-studies-title"><a
                                                            href="https://www.youtube.com/embed/rHupXKF_IPM">Marketing Animation</a></h3>
                                                            <div class="case-studies-content">
                                                                <div class="case-studies-description">Animation</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="https://www.youtube.com/embed/rHupXKF_IPM"><svg
                                                                width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                <path
                                                                d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28"
                                                                stroke="black" stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"></path>
                                                                </svg></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

                                        <div class="grid-item" data-groups='["Animation"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <iframe width="370" height="400" src="https://www.youtube.com/embed/zNmhBT4dQD8" title="Virtual Mom Life Simulator - Happy Family  | Game | Futurealiti" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <h3 class="case-studies-title"><a
                                                            href="https://www.youtube.com/embed/zNmhBT4dQD8">Virtual Mom Life Simulator</a></h3>
                                                            <div class="case-studies-content">
                                                                <div class="case-studies-description">Animation</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="https://www.youtube.com/embed/zNmhBT4dQD8"><svg
                                                                width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                <path
                                                                d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28"
                                                                stroke="black" stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"></path>
                                                                </svg></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

                                        <div class="grid-item" data-groups='["Animation"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <iframe width="370" height="400" src="https://www.youtube.com/embed/Bw0QIxHLirM" title="Heavy Lifting Robot" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <h3 class="case-studies-title"><a
                                                            href="https://www.youtube.com/embed/Bw0QIxHLirM">Heavy Lifting Robot</a></h3>
                                                            <div class="case-studies-content">
                                                                <div class="case-studies-description">Animation</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="https://www.youtube.com/embed/Bw0QIxHLirM"><svg
                                                                width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                <path
                                                                d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28"
                                                                stroke="black" stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"></path>
                                                                </svg></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

                                        <div class="grid-item" data-groups='["Animation"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <iframe width="370" height="400" src="https://www.youtube.com/embed/___mmPMqV74" title="Cartesian Robot and Mechanism - Futurealiti Industrial Automation" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <h3 class="case-studies-title"><a
                                                            href="https://www.youtube.com/embed/___mmPMqV74">Cartesian Robot and Mechanism</a></h3>
                                                            <div class="case-studies-content">
                                                                <div class="case-studies-description">Animation</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="https://www.youtube.com/embed/___mmPMqV74"><svg
                                                                width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                <path
                                                                d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28"
                                                                stroke="black" stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"></path>
                                                                </svg></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["Animation"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <iframe width="370" height="400" src="https://www.youtube.com/embed/UQziEvxHUg8" title="Bridge Vista - 3D animated building plan by Futurealiti" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <h3 class="case-studies-title"><a
                                                            href="https://www.youtube.com/embed/UQziEvxHUg8">Bridge Vista</a></h3>
                                                            <div class="case-studies-content">
                                                                <div class="case-studies-description">Animation</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="https://www.youtube.com/embed/UQziEvxHUg8"><svg
                                                                width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                <path
                                                                d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28"
                                                                stroke="black" stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"></path>
                                                                </svg></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["Animation"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <iframe width="370" height="400" src="https://www.youtube.com/embed/iSiD5qYAA1s" title="Futurealiti - Idea into reality" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <h3 class="case-studies-title"><a
                                                            href="https://www.youtube.com/embed/iSiD5qYAA1s">Futurealiti - Idea into reality</a></h3>
                                                            <div class="case-studies-content">
                                                                <div class="case-studies-description">Animation</div>
                                                            </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="https://www.youtube.com/embed/iSiD5qYAA1s"><svg
                                                                width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                <path
                                                                d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28"
                                                                stroke="black" stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"></path>
                                                                </svg></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div> -->
                                        <!-- End Animation -->

                                        <!-- Start 3D Modeling -->
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-01.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Car Model</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-02.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Character Model</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-03.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Car Model</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-04.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Car Model</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-05.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Building Model</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-06.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Character Model</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-07.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Scary Clown Model</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-08.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Gangster Pose Model</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-09.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Rally Truck Model</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-10.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Proplare Plane Model</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-11.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Building Exterior Structure Model</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-12.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Black Kurta Character Pose</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-13.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Survival Girl Poses</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-14.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Modern Axe Man</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-15.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Hero Thumbs Up Pose</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-16.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Tommy - Horror Game Hero</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-17.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Soccer Main Character</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-18.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Habib Stall</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-19.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Dog Character</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-20.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Round Chair For Packing</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-21.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Packaging Robot</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-22.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Packaging Rebort</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-23.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Packaging Rebort Assembly Line</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-24.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Packaging Rebort</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-25.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Packaging Rebort Assembly Line</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-26.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Packaging Rebort Assembly Line</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-27.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Packaging Rebort Assembly Table</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-28.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Packaging Rebort Assembly Line Motor</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-29.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">tractor Model</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-30.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">TPackaging Robot Assembly Line</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["3d-modeling"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/3d-modal/3d-modal-mockup-31.webp" alt="3D Modeling">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Touch LED Placement Table</a>
                                                            </h3>
                                                            <div class="case-studies-description">3D Modeling</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <!-- End 3D Modeling -->

                                        <!-- Start Game Art -->
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-01.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Angry Dragon Land War Story</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-02.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">AR Flying Numbers</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-03.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Perfect Fresh Juicer Blender</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-04.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Battleground FPS Shooter Game</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-05.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">High Speed Neon Car Endless Driving Simulator Game</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-06.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Fighting Game</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-07.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Zombie Smasher Squad Roadkill</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-08.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Virtual Mom Life Family Simulator</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-09.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Tricky Mystery Mind Puzzles</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-10.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Tiger Simulator</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-11.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Wild Lion Animal Survival Game</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-12.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Crazy Taxi Driver Pick Me Up</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-13.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Snow Leopard Hunting Life Game</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-14.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Virtual Mother House Cleaning</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-15.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Evil Ghost Scary Haunted House</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-16.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Virtual High School Simulator</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-17.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">War Hero Rescue Survivor Pull The Sticks Game</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-18.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">ER Hospital Open Heart Surgery</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-19.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Gangster Mafia Criminal War</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-20.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Mafia War Crime City Game</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-21.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Fruit Blend Ninja Slice Splash</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-22.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Little Builder : House Construction Truck Workshop</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-23.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Realistic Bomb Disposal Squad</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-24.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Home Fruit Real Juice Maker</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                            <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-25.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Wild Panther Animal Family Simulator</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid-item" data-groups='["game-art"]'>
                                                <div class="case-studies-wrapper case-studies-style-1">
                                                <div class="case-studies-img">
                                                    <img class="img-fluid" src="assets/images/portfolio/game-art/game-art-mockup-26.webp" alt="Game Art">
                                                </div>
                                                <div class="case-studies-info">
                                                    <div class="case-studies-info-inner">
                                                        <div class="case-studies-content">
                                                            <h3 class="case-studies-title">
                                                                <a href="javascript:(void)">Flick Soccer</a>
                                                            </h3>
                                                            <div class="case-studies-description">Game Art</div>
                                                        </div>
                                                        <div class="case-studies-link">
                                                            <a class="btn-arrow" href="javascript:(void)">
                                                                <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                                                                    <path d="M12.25 29.75L29.75 12.25M29.75 12.25H14M29.75 12.25V28" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                                </svg>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <!-- End Game Art -->
                                    </div>
                                </div>
                            </div>
                        </div>
                    </section>
                </div>
                <!-- End Projects -->

                <!-- Start Our Testimonial -->
                <section>
                    <div class="container">
                        <div class="row align-items-end justify-content-between">
                            <div class="col-lg-6">
                                <div class="section-title mb-0 wow fadeInUp" data-wow-delay="0.1s" data-wow-duration="1500ms">
                                    <span class="sub-title">[ Testimonials ]</span>
                                    <h2 class="title ">Read what Our Client say about work</h2>
                                </div>
                            </div>
                            <div class="col-lg-6">
                                <p class="p-black mb-0 px-xxl-5 ms-xxl-5 mt-3 mt-lg-0 wow fadeInUp" data-wow-delay="0.1s" data-wow-duration="1500ms">Our clients don’t just see numbers — they experience real growth. From startups to global brands, DigiTroopers turns ambitious ideas into measurable success stories</p>
                            </div>
                        </div>

                        <div class="row justify-content-between mt-4 mt-md-5">
                            <div class="col-lg-4 pe-xxl-5">
                                <div class="action-box wow fadeInUp" data-wow-delay="0.1s" data-wow-duration="1500ms">
                                    <div class="client-reviews">
                                        <ul class="client-profile">
                                            <li>
                                                <img class="img-fluid" src="assets/images/avatar/01.jpg" alt="Testimonials">
                                            </li>
                                            <li>
                                                <img class="img-fluid" src="assets/images/avatar/02.jpg" alt="Testimonials">
                                            </li>
                                            <li>
                                                <img class="img-fluid" src="assets/images/avatar/03.jpg" alt="Testimonials">
                                            </li>
                                        </ul>
                                        <div class="reviews">
                                            <span class="reviews-number text-white"> 1,000+</span>
                                            <span>Client Reviews</span>
                                        </div>
                                    </div>
                                    <div class="action-box-content">
                                        <div class="counter counter-style-1 p-0">
                                            <div class="counter-number">
                                                <h2 class="timer" data-to="4.5" data-speed="2000" data-decimals="1">4.5</h2>
                                            </div>
                                        </div>
                                        <p class="p-white rating-description">Customers are Satisfied with the Technology Services.</p>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-8 mt-4 mt-lg-0 overflow-hidden wow fadeInUp" data-wow-delay="0.1s" data-wow-duration="1500ms">
                                <div class="owl-carousel owl-nav-center slider-overflow" data-nav-arrow="true" data-items="1" data-lg-items="1" data-md-items="1" data-sm-items="1" data-xs-items="1" data-space="24">
                                    <div class="item" data-cursor-type="arrow">
                                        <div class="testimonial-wrapper">
                                            <div class="testimonial-ratings">
                                                <i class="fa-solid fa-star"></i>
                                                <i class="fa-solid fa-star"></i>
                                                <i class="fa-solid fa-star"></i>
                                                <i class="fa-solid fa-star"></i>
                                                <i class="fa-solid fa-star"></i>
                                            </div>
                                            <div class="testimonial-quote">
                                                <i class="fa-solid fa-quote-left"></i>
                                            </div>
                                            <div class="testimonial-content">
                                                <p> “DigiTroopers completely transformed our online visibility. Within months, our website traffic doubled and leads increased steadily. Their SEO strategy really works—it feels like they’re part of our in-house team.”</p>
                                            </div>
                                            <div class="testimonial-author">
                                                <div class="author-info">
                                                    <h3 class="author-name"> SEO Services</h3>
                                                    <span class="author-position">John Miller (New York, USA)</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="item" data-cursor-type="arrow">
                                        <div class="testimonial-wrapper">
                                            <div class="testimonial-ratings">
                                                <i class="fa-solid fa-star"></i>
                                                <i class="fa-solid fa-star"></i>
                                                <i class="fa-solid fa-star"></i>
                                                <i class="fa-solid fa-star"></i>
                                                <i class="fa-solid fa-star"></i>
                                            </div>
                                            <div class="testimonial-quote">
                                                <i class="fa-solid fa-quote-left"></i>
                                            </div>
                                            <div class="testimonial-content">
                                                <p> “Our brand engagement skyrocketed after DigiTroopers took over our social media. They create content that actually connects with people, and we’ve built a loyal customer base.”</p>
                                            </div>
                                            <div class="testimonial-author">
                                                <div class="author-info">
                                                    <h3 class="author-name">Social Media Marketing </h3>
                                                    <span class="author-position">Sarah Johnson (Los Angeles, USA)</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="item" data-cursor-type="arrow">
                                        <div class="testimonial-wrapper">
                                            <div class="testimonial-ratings">
                                                <i class="fa-solid fa-star"></i>
                                                <i class="fa-solid fa-star"></i>
                                                <i class="fa-solid fa-star"></i>
                                                <i class="fa-solid fa-star"></i>
                                                <i class="fa-solid fa-star"></i>
                                            </div>
                                            <div class="testimonial-quote">
                                                <i class="fa-solid fa-quote-left"></i>
                                            </div>
                                            <div class="testimonial-content">
                                                <p> “From SEO to ads and social media, DigiTroopers handle it all. They’ve been a true growth partner, helping us scale nationwide with measurable results.”
                                                </p>
                                            </div>
                                            <div class="testimonial-author">
                                                <div class="author-info">
                                                    <h3 class="author-name"> Complete Digital Marketing</h3>
                                                    <span class="author-position"> Michael Thompson (Miami, USA)</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="item" data-cursor-type="arrow">
                                        <div class="testimonial-wrapper">
                                            <div class="testimonial-ratings">
                                                <i class="fa-solid fa-star"></i>
                                                <i class="fa-solid fa-star"></i>
                                                <i class="fa-solid fa-star"></i>
                                                <i class="fa-solid fa-star"></i>
                                                <i class="fa-solid fa-star"></i>
                                            </div>
                                            <div class="testimonial-quote">
                                                <i class="fa-solid fa-quote-left"></i>
                                            </div>
                                            <div class="testimonial-content">
                                                <p> “The DigiTroopers team built us a website that looks amazing and loads lightning fast. It’s not just design—it’s a tool that drives conversions daily.”</p>
                                            </div>
                                            <div class="testimonial-author">
                                                <div class="author-info">
                                                    <h3 class="author-name"> Web Development</h3>
                                                    <span class="author-position"> Emily Roberts (Houston, USA)</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </section>
                <!-- End Our Testimonial -->

                <!-- Start Footer -->
                <footer class="site-footer footer-dark">
                    <div class="footer-main">
                        <div class="container">
                            <div class="grid-wrapper grid-xs-1 grid-md-2 grid-xl-4">
                                <div class="item">
                                    <a class="footer-logo" href="index">
                                        <img class="img-fluid" src="assets/images/logo/logo.png" alt="Digitroopers Logo" loading="eager">
                                    </a>
                                </div>

                                <div class="item">
                                    <div class="widget">
                                        <h3 class="widget-title">Office</h3>
                                    </div>
                                    <div class="contact-info-box">
                                        <ul>
                                            <li>→ 12316 SE 254th Ct Kent, WA 98030, United States</li>
                                            <li>→ Floor 3, Building 7264, Ibn Al Fourat, Jareer, P.O 12837 Riyadh, Saudi Arabia</li>
                                            <li>
                                                <a href="mailto:info@example.com">→ info@digitroopers.com</a>
                                            </li>
                                            <li>
                                                <a href="tel:1(253)528-4057">→ +1 (253) 528-4057</a>
                                            </li>
                                            <li>
                                                <a href="tel:1-800-555-500">→ +1 (800) 555-500</a>
                                            </li>
                                        </ul>
                                    </div>
                                </div>

                                <div class="item">
                                    <div class="widget-menu">
                                        <h3 class="widget-title">Quick Links</h3>
                                        <ul class="list-unstyled">
                                            <li>
                                                <a href="index">Home</a>
                                            </li>
                                            <li>
                                                <a href="about-us">About Us</a>
                                            </li>
                                            <li>
                                                <a href="portfolio">Portfolio</a>
                                            </li>
                                            <li>
                                                <a  href="insight">Insights</a>
                                            </li>
                                            <li>
                                                <a href="career">Careers</a>
                                            </li>
                                            <li>
                                                <a href="our-process">Our Process</a>
                                            </li>
                                            <li>
                                                <a href="privacy-policy">Privacy Policy</a>
                                            </li>
                                            <li>
                                                <a href="terms-and-conditions">Terms & Conditions</a>
                                            </li>
                                            <li>
                                                <a href="contactus">Contact Us</a>
                                            </li>
                                        </ul>
                                    </div>
                                </div>

                                <div class="item">
                                    <div class="widget-menu">
                                        <h3 class="widget-title">Services</h3>
                                        <ul class="list-unstyled">
                                            <li>
                                                <a href="graphic-design">Graphics Design</a>
                                            </li>
                                            <li>
                                                <a href="seo">Search Engine Optimization</a>
                                            </li>
                                            <li>
                                                <a href="social-media-marketing">Socal Media Markrting</a>
                                            </li>
                                            <li>
                                                <a href="email-marketing">E-mail Marketing</a>
                                            </li>
                                            <li>
                                                <a href="ecommerce">E-Commerce Project Managment</a>
                                            </li>
                                            <li>
                                                <a href="PPC-Managment-Services">Google Ads & PPC</a>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <!-- Start Marquee -->
                        <div class="container-fluid p-4">
                            <div class="row g-0">
                                <div class="col-md-12">
                                    <div class="marquee-wrapper marquee-style-3 marquee-fade">
                                        <div class="marquee-inner">
                                            <div class="marquee-item">
                                                <span class="title">DigiTroopers</span>
                                            </div>
                                            <div class="marquee-item">
                                                <span class="title">DigiTroopers</span>
                                            </div>
                                            <div class="marquee-item">
                                                <span class="title">DigiTroopers</span>
                                            </div>
                                            <div class="marquee-item">
                                                <span class="title">DigiTroopers</span>
                                            </div>
                                            <div class="marquee-item">
                                                <span class="title">DigiTroopers</span>
                                            </div>
                                            <div class="marquee-item">
                                                <span class="title">DigiTroopers</span>
                                            </div>
                                            <div class="marquee-item">
                                                <span class="title">DigiTroopers</span>
                                            </div>
                                            <div class="marquee-item">
                                                <span class="title">DigiTroopers</span>
                                            </div>
                                            <div class="marquee-item">
                                                <span class="title">DigiTroopers</span>
                                            </div>
                                            <div class="marquee-item">
                                                <span class="title">DigiTroopers</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- End Marquee -->
                    </div>

                    <div class="footer-copyright">
                        <div class="container">
                            <div class="row">
                                <div class="col-md-6">
                                    <p class="p-black copyright-info">© Copyrights 2025 DigiTroopers. All Rights Reserved.</p>
                                </div>
                                <div class="col-md-6 mt-2 mt-md-0 text-md-end">
                                    <div class="widget-social">
                                        <span class="follow-title">Follow us</span>
                                        <ul class="social-icon">
                                            <li>
                                                <a href="https://www.facebook.com/DigiTroopers/">Fa.</a>
                                            </li>
                                            <li>
                                                <a href="https://x.com/Digi_Troopers/">Tw.</a>
                                            </li>
                                            <li>
                                                <a href="https://www.instagram.com/digitalmarketingservicesusa/">In.</a>
                                            </li>
                                            <li>
                                                <a href="https://www.linkedin.com/company/digitroopers/">Lin.</a>
                                            </li>

                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </footer>
                <!-- End Footer -->

                <!-- Start Back To Top -->
                <div class="back-to-top">
                    <svg class="progress-circle svg-content" width="100%" height="100%" viewBox="-1 -1 102 102">
                        <path d="M50,1 a49,49 0 0,1 0,98 a49,49 0 0,1 0,-98"/>
                    </svg>
                </div>
                <!-- End Back To Top -->
            </div>
        </div>

        <script src="assets/vendors/jquery/jquery.min.js"></script>
		<script src="assets/vendors/jquery-appear/jquery.appear.js"></script>
		<script src="assets/vendors/popper/popper.min.js"></script>
		<script src="assets/vendors/bootstrap/js/bootstrap.bundle.min.js"></script>
		<script src="assets/vendors/owl-carousel/owl.carousel.min.js"></script>
		<script src="assets/vendors/swiper/swiper.min.js"></script>
		<script src="assets/vendors/wow/wow.min.js"></script>
		<script src="assets/vendors/swiperanimation/SwiperAnimation.min.js"></script>
		<script src="assets/vendors/shuffle/shuffle.min.js"></script>
		<script src="assets/vendors/magnific-popup/jquery.magnific-popup.js"></script>
		<script src="assets/vendors/counter/jquery.countTo.js"></script>
		<script src="assets/js/gsap.min.js"></script>
		<script src="assets/vendors/slideshow/jquery.velocity.min.js"></script>
		<script src="assets/vendors/slideshow/jquery.kenburnsy.js"></script>
		<script src="assets/vendors/lenis/lenis.min.js"></script>
		<script src="assets/vendors/jarallax/jarallax.min.js"></script>
		<script src="assets/js/themes.js"></script>
		<script src="assets/js/custom.js"></script>
    </body>
</html>